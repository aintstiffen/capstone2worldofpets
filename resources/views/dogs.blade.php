                        ...
                        <a href="" class="group">
                            <div class="overflow-hidden border rounded-lg transition-all hover:shadow-lg">
                                <div class="aspect-square relative">
                                    <img src="{{ $pet->image ?? '/placeholder.svg?height=300&width=300' }}" 
                                         alt="{{ $pet->name }}" 
                                         class="object-cover w-full h-full transition-transform group-hover:scale-105">
                                </div>
                                <div class="p-4">
                                    <h2 class="font-bold text-lg truncate">{{ $pet->name }}</h2>
                                    <p class="text-sm text-muted-foreground truncate">{{ $pet->temperament ?? 'N/A' }}</p>
                                </div>
                                <hr>
                                <div class="p-4 flex justify-between items-center text-sm">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-muted-foreground">{{ $pet->lifespan ?? 'N/A' }} years</span>
                                    </div>
