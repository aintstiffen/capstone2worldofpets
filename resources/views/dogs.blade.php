...
@section('content')
    <div class="flex flex-col min-h-screen">
        ...
        <!-- Hero Section -->
        <main class="flex-1">
            <section class="w-full py-6 md:py-24 lg:py-32 paw-pattern"
                style="background: linear-gradient(180deg, color-mix(in oklab, var(--color-secondary) 8%, white), var(--color-muted));">
                        ...
                        @if (isset($pets) && $pets->count() > 0)
                            @foreach ($pets as $pet)
                                <a href="{{ route('dogs.show', $pet->slug) }}" class="group hover-lift">
                                    <div
                                        class="overflow-hidden border rounded-lg transition-all hover:shadow-lg surface-card hover:border-[--color-primary]">
                                        ...
                                        <div class="p-4 flex justify-between items-center text-sm">
                                            <div class="flex items-center gap-2">
                                                <span class="px-2 py-0.5 rounded-full"
                                                    style="background-color: color-mix(in oklab, var(--color-primary) 12%, white); color: var(--color-primary);">{{ $pet->size ?? 'N/A' }}</span>
                                                <span class="text-[var(--color-muted-foreground)]">•</span>
                                                <span class="px-2 py-0.5 rounded-full text-[--color-primary]"
                                                    style="background-color: color-mix(in oklab, var(--color-primary) 10%, white);">{{ $pet->lifespan ?? 'N/A' }}
                                                    years</span>
                                            </div>
                    ...
                    @if ($pets->hasPages())
                        <div class="mt-12 flex justify-center">
                            <nav class="flex items-center gap-2">
                                ...
                                @if ($pets->onFirstPage())
                                    <span
                                        class="px-4 py-2 text-sm font-medium text-[--color-muted-foreground] bg-[--color-muted] border border-gray-300 rounded-lg cursor-not-allowed">
                                        ← Previous
                                    </span>
                                @else
                                    <a href="{{ $pets->previousPageUrl() }}"
                                        class="px-4 py-2 text-sm font-medium text-[--color-foreground] bg-white border border-gray-300 rounded-lg hover:bg-[--color-muted] hover:border-[--color-border] transition-colors">
                                        ← Previous
                                    </a>
                                @endif
                                ...
                                <div class="flex items-center gap-1">
                                    @foreach ($pets->getUrlRange(1, $pets->lastPage()) as $page => $url)
                                        @if ($page == $pets->currentPage())
                                            <span
                                                class="px-4 py-2 text-sm font-bold text-white bg-[--color-primary] border border-[--color-primary] rounded-lg">
                                                {{ $page }}
                                            </span>
                                        @else
                                            <a href="{{ $url }}"
                        ...
                        <div class="mt-4 text-center text-sm text-[--color-muted-foreground]">
                            Showing {{ $pets->firstItem() }} to {{ $pets->lastItem() }} of {{ $pets->total() }} dog
                            breeds
                        </div>
                    @endif
                </div>
