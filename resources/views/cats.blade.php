        ...
        <div class="container mx-auto px-4 md:px-6">
            <div class="text-center max-w-4xl mx-auto animate-fade-in-up">
                ...
                <p class="text-lg md:text-xl text-gray-600 mb-8 leading-relaxed">
                    Explore comprehensive profiles of the most beloved cat breeds in the Philippines. 
        ...
        <div class="container mx-auto px-4 md:px-6">
            @if(isset($petss) && $petss->count() > 0)
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                    @foreach($petss as $index => $pets)
                        <a href="{{ route('cats.show', $pets->slug) }}" 
                           class="breed-card animate-fade-in-up stagger-{{ min($index + 1, 4) }}">
                            ...
                            <div class="breed-card-image">
                                ...
                                <!-- Price Badge -->
                                @if($pets->price_min && $pets->price_max)
                                    <div class="price-badge">
                                        ₱{{ number_format($pets->price_min) }} - ₱{{ number_format($pets->price_max) }}
                                    </div>
                                @elseif($pets->price_range)
                                    <div class="price-badge">
                                        {{ $pets->price_range }}
                                    </div>
                                @endif
                            ...
                            <div class="p-6 flex-1 flex flex-col">
                                <!-- Breed Name -->
                                <h2 class="text-2xl font-bold mb-2 text-gray-900">{{ $pets->name }}</h2>
                                
                                <!-- Temperament -->
                                <p class="text-sm text-gray-600 mb-4 line-clamp-2">
                                    {{ Str::limit($pets->temperament ?? $pets->description ?? 'Loving and affectionate companion', 80) }}
                                </p>
                                ...
                                <div class="mt-auto pt-4 border-t border-gray-100 space-y-2">
                                    @if($pets->origin)
                                        <div class="flex items-center gap-2 text-sm text-gray-600">
                                            ...
                                            <span>{{ $pets->origin }}</span>
                                        </div>
                                    @endif
                                    
                                    @if($pets->energy_level)
                                        <div class="flex items-center gap-2 text-sm text-gray-600">
                                            <span class="text-lg">⚡</span>
                                            <span class="font-medium">Energy:</span>
                                            <span>{{ $pets->energy_level }}</span>
                                        </div>
                                    @endif
                                    
                                    @if($pets->friendliness)
                                        <div class="flex items-center gap-2 text-sm text-gray-600">
                                            <span class="text-lg">😊</span>
                                            <span class="font-medium">Friendly:</span>
                                            <div class="flex gap-0.5">
                                                @for($i = 1; $i <= 5; $i++)
                                                    <svg class="w-4 h-4 {{ $i <= $pets->friendliness ? 'text-yellow-400' : 'text-gray-300' }}" 
            ...
            @else
                ...
                <div class="empty-state animate-fade-in-up">
                    ...
                    <h3 class="text-2xl font-bold mb-3 text-gray-900">No Cat Breeds Found</h3>
                    <p class="text-gray-600 mb-6 max-w-md mx-auto">
                        We're currently updating our database with amazing cat breeds. Please check back soon!
                    </p>
                    <a href="{{ route('home') }}" 
            ...
            @if($petss->hasPages())
                <div class="mt-16">
                    ...
                    <nav class="flex justify-center items-center gap-2 mb-6">
                        {{-- Previous Page --}}
                        <button onclick="window.location='{{ $petss->previousPageUrl() }}'" 
                                class="pagination-btn {{ $petss->onFirstPage() ? 'opacity-50 cursor-not-allowed' : 'bg-white hover:bg-gray-50' }} border border-gray-300"
                                {{ $petss->onFirstPage() ? 'disabled' : '' }}>
                        ...
                        <div class="flex items-center gap-2">
                            @foreach ($petss->getUrlRange(1, $petss->lastPage()) as $page => $url)
                                <button onclick="window.location='{{ $url }}'" 
                                        class="pagination-btn {{ $page == $petss->currentPage() ? 'active' : 'bg-white hover:bg-gray-50 border border-gray-300' }}">
                                    {{ $page }}
                                </button>
                            @endforeach
                        </div>
                        ...
                        {{-- Next Page --}}
                        <button onclick="window.location='{{ $petss->nextPageUrl() }}'" 
                                class="pagination-btn {{ !$petss->hasMorePages() ? 'opacity-50 cursor-not-allowed' : 'bg-white hover:bg-gray-50' }} border border-gray-300"
                                {{ !$petss->hasMorePages() ? 'disabled' : '' }}>
                            Next
                    ...
                    <div class="text-center">
                        <p class="text-sm text-gray-600 bg-white inline-block px-6 py-3 rounded-full shadow-sm">
                            Showing <span class="font-semibold text-[#667eea]">{{ $petss->firstItem() }}</span> 
                            to <span class="font-semibold text-[#667eea]">{{ $petss->lastItem() }}</span> 
                            of <span class="font-semibold text-[#667eea]">{{ $petss->total() }}</span> cat breeds
                        </p>
                    </div>
                </div>
            @endif
